    ...
    if ($hasil) {
        if ($passwordlama == $hasil['password']) {
            if ($passwordbaru == $repasswordbaru) {
                $query = mysqli_query($conn, "UPDATE tb_user SET password='$passwordbaru' WHERE username = '$_SESSION[username_foodienow]'");
                if ($query) {
        ...
        $message = '<script>alert("Username tidak ditemukan")
                        window.history.back()</script>
                        </script>';
    }
} else {
